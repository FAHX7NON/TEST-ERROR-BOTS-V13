...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/8fdcfd9544fcbd364acd7.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Reelly-Bot Multidevice', body: 'I Think, You Are Thinking', sourceUrl: 'https://youtube.com/@ReellyXD', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
